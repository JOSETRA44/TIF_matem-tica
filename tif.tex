...
\niveltres{4.1.3. Validación de Estabilidad y Matriz Inversa}

Previo al cálculo de la inversa, es imperativo verificar las condiciones de viabilidad económica de Hawkins-Simon. Para ello, calculamos el determinante de la matriz de Leontief $(I-A)$.

\begin{equation}
\det(I-A) \approx \textbf{0.5567}
\end{equation}

Dado que el determinante es estrictamente positivo ($\det > 0$) y los elementos de la diagonal principal de $(I-A)$ son positivos, se cumple la condición de estabilidad, garantizando que el sistema tiene una solución no negativa única. Procediendo a la inversión matricial con los datos depurados:
...
\niveltres{4.2.1. Formulación del Modelo Matemático}
...
\niveltres{4.2.2. Definición de la Restricción Presupuestaria}
...
\niveltres{4.2.3. Aplicación del Método de Multiplicadores de Lagrange}

Construimos la función Lagrangiana $\mathcal{L}$ para incorporar la restricción a la función objetivo:

\begin{equation}
\mathcal{L}(s, e, \lambda) = s^{0.4}e^{0.6} - \lambda(s + e - 1207.2451)
\end{equation}

Las condiciones de primer orden (CPO) establecen que la relación marginal de sustitución debe igualar a la relación de precios (que en este caso es 1):

\begin{equation}
\frac{U_e}{U_s} = \frac{0.6s}{0.4e} = 1 \implies e = 1.5s
\end{equation}

\niveltres{4.2.4. Resolución del Sistema}

Sustituyendo la senda de expansión ($e = 1.5s$) en la restricción presupuestaria:

\begin{equation}
s + 1.5s = 1207.2451 \implies 2.5s = 1207.2451
\end{equation}

Despejando las variables endógenas obtenemos la asignación óptima precisa:

\begin{equation}
s^* = \frac{1207.2451}{2.5} = \textbf{482.90 \text{ Millones S/.}}
\end{equation}

\begin{equation}
e^* = 1207.2451 - 482.898 = \textbf{724.35 \text{ Millones S/.}}
\end{equation}
...
Evaluando en el punto crítico $(s^*=482.9, e^*=724.3)$ con los valores de alta precisión:
...
\begin{equation}
|\bar{H}| \approx 0.00176 > 0
\end{equation}

Dado que el determinante del Hessiano Orlado es positivo, el punto encontrado corresponde a un máximo local, validando la solución.

Con respecto al multiplicador de Lagrange, el valor obtenido de $\lambda^* \approx 0.5102$ representa el precio sombra de la restricción presupuestaria. Económicamente, esto indica que por cada millón de soles adicional que ingrese al presupuesto del Canon Minero, el bienestar social agregado se incrementará en 0.51 unidades, lo que confirma la eficiencia marginal positiva de la inversión pública en los niveles actuales.

\niveldos{4.3. Análisis de Sensibilidad y Estática Comparativa}

Una vez determinado el equilibrio, es fundamental analizar la respuesta de la asignación óptima ante variaciones en el presupuesto ($C$). Aplicamos la diferenciación total del sistema de condiciones de primer orden y resolvemos mediante la Regla de Cramer utilizando el Hessiano Orlado.

\niveltres{4.3.1. Planteamiento Matricial}

Al diferenciar el sistema respecto al parámetro $C$, obtenemos un sistema lineal donde el vector de términos independientes $[1, 0, 0]^T$ representa el aumento unitario en la restricción presupuestaria:
...
Utilizamos la matriz Hessiana Orlada ($\bar{H}$) evaluada previamente, cuyo determinante es $|\bar{H}| \approx 0.00176$.

\niveltres{4.3.2. Resolución por Regla de Cramer}

Para hallar cómo cambia la asignación de Salud ($s$) y Educación ($e$) ante un cambio en el presupuesto, sustituimos la columna correspondiente por el vector de términos independientes y dividimos por el determinante del sistema:
...
Los resultados obtenidos confirman rigurosamente que las propensiones marginales a invertir son constantes e indican una regla de política fiscal invariable: por cada sol adicional que ingrese a las arcas regionales, exactamente 40 céntimos deben dirigirse a cerrar brechas en Salud y 60 céntimos a Educación para mantener el bienestar social en su nivel máximo.

\niveltres{4.3.3. Simulación de Escenario}

Si el Canon Minero aumentara en $\Delta C = 100$ millones de soles debido a un ciclo de precios altos, la reasignación eficiente de recursos, siguiendo los multiplicadores hallados, sería:
...
Esta simulación demuestra la robustez del modelo, pues ante shocks presupuestarios positivos, la distribución no debe ser discrecional, sino que debe seguir estrictamente la estructura de preferencias revelada por la función de bienestar social.
