...
La investigación utiliza como matriz base la \textbf{Matriz Insumo-Producto (MIP) Total 2007} del INEI, por ser la referencia estándar vigente para cuentas nacionales[cite: 470, 471]. Para la regionalización, se emplean los datos del \textbf{Valor Agregado Bruto (VAB)} de Arequipa proyectados al 2024[cite: 488, 492].
...
\end{tabular}
\end{table}
\textit{Fuente: Elaboración propia con datos del BCRP e INEI[cite: 493].}

\vspace{0.3cm}

\niveldosnotoc{Anexo 2: Regionalización y Estabilidad}

Se aplicó el \textbf{Coeficiente de Ubicación de Flegg (FLQ)} para ajustar la matriz nacional a la estructura de Arequipa[cite: 512, 513].
...
\end{tabular}
\end{table}
\textit{Fuente: Cálculos propios detallados en el Capítulo IV[cite: 527, 559].}
...
// Verificacion de estabilidad y calculo de inversa [cite: 583, 589]
det_L = det(Leontief_Matrix);
Leontief_Inverse = inv(Leontief_Matrix);

// Definicion del shock en Mineria (Sector 2) [cite: 599]
delta_d = zeros(9,1); 
delta_d(2) = 981.8; 

// Calculo del impacto en la produccion total (delta_X) [cite: 602]
