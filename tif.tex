...
La presente investigación evalúa el impacto económico de la minería formal en la región de Arequipa durante el periodo 2024 y propone una asignación óptima de los recursos provenientes del Canon y Regalías Mineras. Utilizando un modelo Insumo-Producto de Wassily Leontief, regionalizado mediante el método del Coeficiente de Ubicación de Flegg (FLQ) sobre 9 sectores productivos, se determinó que el multiplicador de producción del sector minero es de 1.080. Este valor evidencia una estructura de ``enclave'', con encadenamientos débiles hacia la manufactura y agricultura local. Ante esta realidad, se aplicó la técnica de optimización de Multiplicadores de Lagrange para distribuir un presupuesto real de 1,207.25 millones de soles. Los resultados indican que la asignación óptima para maximizar el bienestar social consiste en destinar 482.90 millones a Salud y 724.35 millones a Educación. El análisis de sensibilidad confirmó una regla de asignación marginal de 40/60, validando las herramientas de matemáticas aplicadas como instrumentos eficaces para la gestión pública y la toma de decisiones técnicas en la región.

\textbf{Palabras clave:} Insumo-Producto, Canon Minero, Multiplicadores de Lagrange, Optimización, Arequipa, Economía de Enclave.
...
\niveldosnotoc{Anexo 1: Base de Datos y Fuentes}

La investigación utiliza como matriz base la \textbf{Matriz Insumo-Producto (MIP) Total 2007} del INEI, por ser la referencia estándar vigente para cuentas nacionales. Para la regionalización, se emplean los datos del \textbf{Valor Agregado Bruto (VAB)} de Arequipa proyectados al 2024.

\begin{table}[H]
\centering
\caption{VAB de Arequipa por Actividad Económica, 2024}
\small
\begin{tabular}{lrr}
\toprule
\textbf{Actividad Económica} & \textbf{VAB (Miles S/)} & \textbf{Estructura (\%)} \\
\midrule
Agricultura, Ganadería, Caza y Silv. & 1,682,946 & 5.3 \\
\textbf{Extracción de Petróleo y Minerales} & \textbf{10,125,582} & \textbf{32.2} \\
Manufactura & 3,276,801 & 10.4 \\
Electricidad, Gas y Agua & 325,403 & 1.0 \\
Construcción & 2,529,854 & 8.0 \\
Comercio & 3,236,853 & 10.3 \\
Transporte y Comunicaciones & 3,070,421 & 9.8 \\
Otros Servicios & 7,204,594 & 23.0 \\
\midrule
\textbf{VAB Total} & \textbf{31,458,086} & \textbf{100.0} \\
\bottomrule
\end{tabular}
\end{table}
...
\niveldosnotoc{Anexo 2: Regionalización y Estabilidad}

Se aplicó el \textbf{Coeficiente de Ubicación de Flegg (FLQ)} para ajustar la matriz nacional a la estructura de Arequipa.

\begin{table}[H]
\centering
\caption{Cálculo del Coeficiente de Ubicación (FLQ) y Validación de Estabilidad}
\small
\begin{tabular}{lc|lc}
\toprule
\textbf{Sector (Minería)} & \textbf{Valor} & \textbf{Criterio de Estabilidad} & \textbf{Resultado} \\
\midrule
Participación Aqp (\%) & 32.2 & Determinante $|I-A|$ & 0.5567 \\
Participación Nac (\%) & 12.1 & Condición Hawkins-Simon & Cumple \\
\textbf{LQ Resultante} & \textbf{2.66} & \textbf{Viabilidad del Modelo} & \textbf{SI} \\
\bottomrule
\end{tabular}
\end{table}
\textit{Fuente: Cálculos propios detallados en el Capítulo IV.}

\niveldosnotoc{Anexo 3: Implementación en Scilab}

\begin{lstlisting}[caption={Codigo Scilab para calculos del modelo regional}]
// Definicion de la Matriz A (9x9) y Matriz Identidad I
A = [...]; 
I = eye(9,9); 
Leontief_Matrix = I - A;

// Verificacion de estabilidad y calculo de inversa
det_L = det(Leontief_Matrix);
Leontief_Inverse = inv(Leontief_Matrix);

// Definicion del shock en Mineria (Sector 2)
delta_d = zeros(9,1); 
delta_d(2) = 981.8; 

// Calculo del impacto en la produccion total (delta_X)
delta_X = Leontief_Inverse * delta_d;
disp("Impacto Total (delta_X):", delta_X);
\end{lstlisting}
